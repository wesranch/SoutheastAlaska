...
sp_table <- read.csv("example_data/DGS_Spp_Table-WR-Updated.csv") %>%
  mutate(FunctionalGroupIndex = case_when(
    SpeciesCode %in% c("BlackSpruce", "WhiteSpruce", "Tamarack") ~ 1,
    SpeciesCode %in% c("QuakingAspen", "PaperBirch", "BalsamPoplar") ~ 2,
    SpeciesCode %in% c("Willow", "Alder", "ShrubBirch") ~ 3,
    SpeciesCode %in% c("Sphagnum", "Feathermoss", "Turfmoss") ~ 4,
    TRUE ~ NA_real_
  ))
...
#grp1_names <- c("Picea mariana", "Picea glauca", "Larix laricina")
#grp2_names <- c("Populus tremuloides", "Betula papyrifera", "Populus balsamifera")
#grp3_names <- c("Salix alaxensis", "Alnus incana", "Betula glandulosa")
#grp4_names <- c("Sphagnum fuscum", "Hylocomium splendens", "Rhytidiadelphus squarrosus")
...
conifers <- c(95, 94, 71)
hardwoods <- c(746, 741, 376)
shrubs <- c(920, 5188, 370)

conifer_grcds <- unique(trees$SPGRPCD[trees$SPCD%in%conifers])
hardwoods_grcds <- unique(trees$SPGRPCD[trees$SPCD%in%hardwoods])
shrub_grcds <- unique(trees$SPGRPCD[trees$SPCD%in%shrubs])
...
spgrp_lai <- data.frame(SPGRPCD = unique(trees$SPGRPCD),
  ...
  arrange(SPGRPCD) %>%
  mutate(FunctionalGroupIndex = ifelse(SPGRPCD %in% conifer_grcds, 1, FunctionalGroupIndex))%>% 
  mutate(FunctionalGroupIndex = ifelse(SPGRPCD %in% hardwoods_grcds, 2, FunctionalGroupIndex))%>%
  # mutate(FunctionalGroupIndex = ifelse(SPGRPCD %in% c(), 3, FunctionalGroupIndex)) %>% #shrubs
  # mutate(FunctionalGroupIndex = ifelse(SPGRPCD %in% c(), 4, FunctionalGroupIndex)) %>% #mosses
  left_join(sp_table %>% dplyr::select(FunctionalGroupIndex, KLAI, MaximumLAI))

sp_ref$SPECIES_SYMBOL[sp_ref$SPCD%in%conifers]
sp_ref$SPECIES_SYMBOL[sp_ref$SPCD%in%hardwoods]
sp_ref$SPECIES_SYMBOL[sp_ref$SPCD%in%shrubs]

sp_table <- sp_table %>%
  mutate(SpeciesCodeActual = case_when(
    SpeciesCode == "BlackSpruce" ~ "LALA",
    SpeciesCode == "WhiteSpruce" ~ "PIGL",
    SpeciesCode == "Tamarack" ~ "PIMA",
    SpeciesCode == "QuakingAspen" ~ "BENE4",
    SpeciesCode == "PaperBirch" ~ "POBA2",
    SpeciesCode == "BalsamPoplar" ~ "POTR5",
    SpeciesCode == "Willow" ~ "SALIX",
    SpeciesCode == "Alder" ~ "ALIN2",
    SpeciesCode == "ShrubBirch" ~ "BETUL",
    TRUE ~ NA_character_
  ))

spp_to_use_all <- sp_table$SpeciesCodeActual[1:9]
...
write.csv(plot_leaf_area, "seedlings_with_leaf_area.csv")
